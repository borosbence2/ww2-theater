...
| [Wikidata](https://www.wikidata.org/) commanders — **in use (Phase 4.3 + ext)** | Unit commanders (P598 "commander of") + descriptions + Wikipedia links | CC0 | Two passes: `fetch-commanders.mjs` keyed by the QIDs units already carry (mostly divisions); `fetch-commanders-ext.mjs` resolves the higher formations that have **no** QID — Soviet fronts/armies and German armies/army groups — by label search, verified against claims (country, type, era, ordinal) to reject wrong-nation/wrong-era homonyms. Lifts front/army commander coverage from ~0 to ~90% on both sides and adds Wikidata/Wikipedia links + a description note. Commander tenure dates are often absent on Wikidata (rendered "dates unknown"); curated and QID-dated successions always win. |
